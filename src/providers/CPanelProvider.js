...
/**
 * Provider para integração com servidores cPanel
 */
class CPanelProvider extends BaseProvider {
  constructor(config = {}) {
    super(config);
    
    // Configurações específicas para cPanel
    this.username = config.cpanelUsername || '';
    this.password = config.cpanelPassword || '';
    this.domain = config.cpanelDomain || '';
    this.apiToken = config.cpanelApiToken || '';
    this.port = config.cpanelPort || 2083;
    this.useSSL = config.cpanelUseSSL !== false;
    
    // Base URL da API cPanel
    this.apiBaseUrl = this.useSSL
      ? `https://${this.domain}:${this.port}/execute`
      : `http://${this.domain}:${this.port}/execute`;
  ...
  /**
   * Executa um comando na API do cPanel
   */
  execute(payload) {
    // Se não tiver credenciais, usa o mock provider
    if ((!this.username || !this.password) && !this.apiToken && this.config.fallbackEnabled) {
    ...
    try {
      ...
      // Executa a requisição real para a API do cPanel
  ...
  isAvailable() {
    ...
    if ((!this.username || !this.password) && !this.apiToken) return false;
  ...
  /**
   * Mapeia ações do MCP para módulos/funções do cPanel
   * @private
   */
  _mapActionToCPanelModule(action) {
    const actionMap = {
      'createSite': { module: 'SubDomain', function: 'addsubdomain' },
      'uploadFile': { module: 'Fileman', function: 'upload_files' },
      'getDomainInfo': { module: 'DomainInfo', function: 'domains_data' },
      'deployFiles': { module: 'Fileman', function: 'upload_files' }
      // Adicionar mais mapeamentos conforme necessário
    };
    
    return actionMap[action] || { module: 'API', function: 'exec' };
  }
  
  /**
   * Faz uma requisição para a API do cPanel
   * @private
   */
  _makeApiRequest(payload) {
    // Implementação real da chamada de API do cPanel usando fetch/axios/etc
    // Esta é uma implementação simulada para exemplo
    
    // Mapeia a ação para módulo/função do cPanel
    const { module, function: func } = this._mapActionToCPanelModule(payload.action);
    
    // Simula uma chamada de API
    if (Math.random() > 0.9) {
      // Simula falha ocasional para testar fallback
      throw new Error('cPanel API request failed');
    }
    
    // Simula resposta de API
    return {
      success: true,
      requestId: `cpanel_req_${Math.floor(Math.random() * 1000000)}`,
      timestamp: new Date().toISOString(),
      data: {
        // Dados específicos com base no payload
        module: module,
        function: func,
        message: 'cPanel API request successful',
  ...
  /**
   * Inicia uma operação de stream com a API do cPanel
   * @private
   */
      ...
      setTimeout(() => {
        ...
        const eventData = {
          ...
          data: {
            ...
            details: `Processing ${payload.action} on cPanel`
          },
          timestamp: new Date().toISOString()
        };
